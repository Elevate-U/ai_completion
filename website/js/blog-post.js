...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!postTitleElement || !postDateElement || !postAuthorElement || !postBodyElement || !articleElement) {
    console.error('One or more required post elements not found!');
    if (articleElement) {
        articleElement.innerHTML = '<p>Error: Could not load post content due to a page structure issue.</p>';
    }
    return;
  }
  ...
  if (!slug) {
    ...
    articleElement.innerHTML = '<h1>Post Not Found</h1><p>No specific post was requested. Please return to the <a href="blog.html">blog list</a>.</p>';
  ...
  fetch('data/blog_posts.json') // Corrected path relative to blog-post.html
    ...
    .catch(error => {
      console.error('Error fetching or processing blog post:', error);
       articleElement.innerHTML = '<h1>Error Loading Post</h1><p>There was an error loading the blog post content. Please try again later or return to the <a href="blog.html">blog list</a>.</p>';
