...
export async function renderPricingTabContent(tool) {
  ...
  const hasScrapedTables = pricingData?.scraped_pricing_tables && Array.isArray(pricingData.scraped_pricing_tables) && pricingData.scraped_pricing_tables.length > 0;
  const dataSource = hasScrapedTables ? 'scraped_pricing_tables' : 'original pricing object';

  console.log(`PricingTab: Rendering pricing for "${tool.name}". Using data source: ${dataSource}.`);
