...
export function renderPricingTable(pricingData) {
  // pricingData is now the whole pricing object from JSON
  if (
    !pricingData ||
  ...
  if (
    ...
    const tablesHtml = scrapedTables
      .map((tableObject, tableIndex) => {
        ...
        const tableTitle =
          tableObject.title || `Pricing Table ${tableIndex + 1}`;
        ...
        const headersHtml = (() => {
          try {
            ...
            if (typeof tableData[0] !== 'object' || tableData[0] === null) {
              console.error(
                `PricingComponents: First row of table "${tableTitle}" is not an object:`,
                tableData[0]
              );
            ...
            console.error(
              `PricingComponents: Error generating headers for table "${tableTitle}":`,
              e
            );
            return '<th>Error</th>';
          }
            ...
            return tableData
              .map((row, rowIndex) => {
                if (typeof row !== 'object' || row === null) {
                  console.error(
                    `PricingComponents: Row ${rowIndex} in table "${tableTitle}" is not an object:`,
                    row
                  );
                  // Attempt to get number of headers for colspan, default to a large number
                  const headerCount =
                    typeof tableData[0] === 'object' && tableData[0] !== null
                ...
                const cellsHtml = headers
                  .map((header) => `<td>${renderValue(row[header])}</td>`)
                  .join(''); // Use helper to render value
                return `<tr>${cellsHtml}</tr>`;
              })
              .join('');
          } catch (e) {
            console.error(
              `PricingComponents: Error generating table body for table "${tableTitle}":`,
              e
            );
    ...
    console.log(
      'No valid scraped data found, rendering original pricing object as tiers.'
    );
    ...
    if (
      pricingData.paid_tiers &&
      Array.isArray(pricingData.paid_tiers) &&
      pricingData.paid_tiers.length > 0
    ) {
      fallbackHtml += `
            <div class="tab-section">
