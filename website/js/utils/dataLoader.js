...
function constructFilename(toolId) {
  const fileName = dataLoaderConfig.toolIdToFilenameMap[toolId] || (() => {
    const decodedToolId = decodeURIComponent(toolId);
    return decodedToolId.toLowerCase().replace(/[^a-z0-9\s]/gi, '').replace(/\s+/g, '_') + '.json';
  })();
  return fileName;
}
...
export async function loadToolData() {
  ...
  try {
    ...
    console.log(`Loaded tool data for "${toolData.name}" from ${filePath}. Pricing data should be pre-processed.`);

    return toolData;
  } catch (error) {
    console.error(`Direct fetch failed: ${error}`);
    throw new Error(`Tool data for "${toolId}" could not be loaded. ${error.message}`);
  }
}
...
export async function loadAllToolsData() {
  ...
  try {
    ...
    console.log(`[dataLoader] Received ${toolFilenames.length} tool filenames from server.`);
  ...
  if (toolFilenames.length === 0) {
    console.warn('[dataLoader] No tool filenames found or fetched. Cannot load tool data.');
    ...
    return fetchWithRetry(filePath)
      .then(response => {
        try {
          ...
          console.error(`[dataLoader] Failed to parse JSON from ${fileName}:`, parseError, 'Raw text:', response.text);
  ...
  try {
    ...
    console.log(`[dataLoader] Successfully loaded data for ${allToolsData.length} tools.`);
    ...
    if (allToolsData.length < expectedCount) {
        console.warn(`[dataLoader] Some tool files failed to load or parse. Expected ${expectedCount}, got ${allToolsData.length}.`);
    }
    return allToolsData;
  } catch (error) {
...
// Test cases
console.assert(testFilenameConstruction('Dialogflow (Google Conversational AI)') === 'dialogflow.json', 'Test Case 1 Failed: Dialogflow');
console.assert(testFilenameConstruction('Amazon Lex') === 'amazon_lex.json', 'Test Case 2 Failed: Amazon Lex');
console.assert(testFilenameConstruction('Google Cloud NLP') === 'google_cloud_nlp.json', 'Test Case 3 Failed: Google Cloud NLP');
