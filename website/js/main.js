...
let currentFilters = {
  // To keep track of active filters
  searchTerm: '',
  categories: new Set(),
  pricing: 'all',
  features: new Set(),
...
const pricingFiltersContainer = document.getElementById(
  'pricing-filters-container'
);
const featureFiltersContainer = document.getElementById(
...
function populateFilters(tools) {
  ...
  tools.forEach((tool) => {
    ...
    if (tool.common_features && typeof tool.common_features === 'object') {
      Object.keys(tool.common_features).forEach((feature) =>
...
function displayTools(toolsData) {
  ...
  if (!toolsData || toolsData.length === 0) {
    toolsListContainer.innerHTML =
      '<p class="empty-state">No tools match your criteria.</p>';
    return;
  }
...
function applyFiltersAndSort() {
  ...
  toolsListContainer.innerHTML =
    '<p class="loading-indicator">Filtering...</p>';
  ...
  setTimeout(() => {
    ...
    if (currentFilters.pricing !== 'all') {
      filteredTools = filteredTools.filter((tool) => {
        ...
        const toolPricingModel =
          tool.pricing?.model?.toLowerCase() ||
          tool.pricing?.type?.toLowerCase() ||
          '';
        const filterPricing = currentFilters.pricing;

        if (filterPricing === 'free') {
          return (
            toolPricingModel.includes('free') &&
            !toolPricingModel.includes('paid') &&
            !toolPricingModel.includes('freemium')
          );
        }
        ...
        if (filterPricing === 'paid') {
          ...
          return (
            (toolPricingModel.includes('paid') ||
              toolPricingModel.includes('subscription') ||
              toolPricingModel.includes('one-time')) &&
            !toolPricingModel.includes('free tier only')
          ); // Example exclusion
        }
    ...
    if (currentFilters.features.size > 0) {
      filteredTools = filteredTools.filter((tool) => {
        ...
        if (tool.common_features && typeof tool.common_features === 'object') {
          Object.keys(tool.common_features).forEach((f) =>
            toolFeatures.add(f.trim())
          );
        }
    ...
    try {
      ...
      if (categoryFiltersContainer) {
        categoryFiltersContainer.addEventListener('change', (e) => {
          if (
            ...
            e.target.dataset.filterType === 'category'
          ) {
            if (e.target.checked) {
      ...
      if (featureFiltersContainer) {
        featureFiltersContainer.addEventListener('change', (e) => {
          if (
            e.target.type === 'checkbox' &&
            e.target.dataset.filterType === 'feature'
          ) {
            if (e.target.checked) {
