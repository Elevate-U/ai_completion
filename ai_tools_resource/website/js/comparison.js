...
const featuresChartCtx = document.getElementById('features-chart')?.getContext('2d'); // Use optional chaining
const comparisonChart2Ctx = document.getElementById('pricing-chart')?.getContext('2d'); // Renamed ID for clarity, use optional chaining
...
function initComparisonPage() {
    ...
    if (typeof aiToolsData === 'undefined' || aiToolsData.length === 0) {
        ...
        if (comparisonPlaceholder) {
            comparisonPlaceholder.innerHTML = '<p>Error loading tool data. Please try refreshing.</p>';
    ...
    if (!featuresChartCtx || !comparisonChart2Ctx) {
         console.warn("One or both chart canvas contexts not found. Charts will be disabled.");
    }
...
function populateToolList() {
    ...
    if (filteredTools.length === 0) {
        toolList.innerHTML = '<p class="text-muted p-2">No tools match filters.</p>';
        return;
    }
    ...
    toolList.innerHTML = filteredTools.map(tool => {
        ...
        const isDisabled = !isChecked && selectedTools.length >= MAX_COMPARE_TOOLS;
...
function updateComparisonResults() {
    ...
    const toolsToCompare = aiToolsData.filter(tool => selectedTools.includes(tool.id || tool.name));
    ...
    if (featuresChartCtx && comparisonChart2Ctx) { // Only update charts if contexts exist
...
function updateComparisonTable(tools) {
    ...
    const comparisonAspects = [
        ...
        { label: 'Key Features', key: 'features', format: (val) => val ? val.join(', ') : '-' },
        { label: 'Pricing Model', key: 'pricing.model', fallback: 'N/A' }, // Access nested property safely
        { label: 'Free Tier', key: 'pricing.free_tier', format: (val) => val ? 'Yes' : 'No', fallback: 'N/A' },
        { label: 'Pros', key: 'pros', format: (val) => val ? val.join(', ') : '-' },
        { label: 'Cons', key: 'cons', format: (val) => val ? val.join(', ') : '-' },
        { label: 'API Type', key: 'technical_specifications.api_type', fallback: 'N/A' },
        { label: 'Scalability', key: 'scalability', fallback: 'N/A' },
        { label: 'Support Options', key: 'support_options', format: (val) => val ? val.join(', ') : '-' },
        { label: 'Last Updated', key: 'last_updated', format: (val) => val ? new Date(val).toLocaleDateString() : '-', fallback: 'N/A' },
    ];
    ...
    return `
        <tr>
            ...
            ${values.map(value => {
                ...
                if (truncateLength && typeof displayValue === 'string' && displayValue.length > truncateLength) {
                    displayValue = displayValue.substring(0, truncateLength) + '...';
                }
...
function updateCharts(tools) {
    ...
    featuresChartInstance = new Chart(featuresChartCtx, {
        ...
        options: {
            ...
            plugins: {
                title: { display: true, text: 'Feature Count Comparison', font: { size: 16 } },
    ...
    const radarAspects = [
        ...
        { label: 'Feature Richness', getValue: (tool) => tool.features ? tool.features.length : 0 },
        { label: 'Scalability Score', getValue: getScalabilityScore }, // Higher is better
        { label: 'Support Score', getValue: (tool) => tool.support_options ? tool.support_options.length * 2 : 0 } // Simple score
    ];
    ...
    comparisonChart2Instance = new Chart(comparisonChart2Ctx, {
        ...
        data: {
            ...
            datasets: tools.map((tool, index) => {
                ...
                return {
                    label: tool.name,
                    data: radarAspects.map(aspect => Math.min(10, Math.max(0, aspect.getValue(tool)))), // Get value, clamp between 0-10
        ...
        options: {
            ...
            maintainAspectRatio: false,
            plugins: {
                title: { display: true, text: 'Qualitative Comparison', font: { size: 16 } },
...
function getPricingScore(tool) {
    ...
    if (model.includes('low') || model.includes('affordable') || model.includes('pay-as-you-go')) return 7;
...
function getScalabilityScore(tool) {
    ...
    if (scalability.includes('high') || scalability.includes('excellent')) return 10;
    ...
    if (toolList) {
        toolList.addEventListener('change', (e) => {
            if (e.target.type === 'checkbox') {
                ...
                if (e.target.checked) {
                    if (!selectedTools.includes(toolId) && selectedTools.length < MAX_COMPARE_TOOLS) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let checkCounterCompare = 0; // Counter for checks
    const maxChecksCompare = 50; // Limit checks to 5 seconds

    const checkDataIntervalCompare = setInterval(() => {
        checkCounterCompare++;
        console.log(`[comparison.js] Checking for aiToolsData... Attempt: ${checkCounterCompare}`);

        if (typeof aiToolsData !== 'undefined' && aiToolsData.length >= 0) { // Check >= 0 to handle empty data case
            console.log("[comparison.js] aiToolsData found (or empty array ready), clearing interval and initializing.");
            clearInterval(checkDataIntervalCompare);
            initComparisonPage();
        } else if (checkCounterCompare >= maxChecksCompare) {
             console.error("[comparison.js] Timeout waiting for aiToolsData. Clearing interval.");
             clearInterval(checkDataIntervalCompare);
             // Optionally display an error message on the page here
             if (comparisonPlaceholder) {
                comparisonPlaceholder.innerHTML = '<p>Timeout loading data. Please refresh.</p>';
                comparisonPlaceholder.style.display = 'block';
             }
             if (comparisonResults) comparisonResults.style.display = 'none';
        }
